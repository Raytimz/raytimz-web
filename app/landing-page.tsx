...
import type { LandingCopy } from './translations';
...
export default function LandingPage({ copy }: { copy: LandingCopy }) {
  ...
  return (
    <main lang={copy.locale} onPointerMove={trackPointer}>
        ...
        <div className="hero-intro">
          <div className="intro-meta">
            ...
            <nav className="language-switcher" aria-label={copy.languageLabel}>
              {copy.locale === 'en' ? <span aria-current="page">EN</span> : <Link href="/" hrefLang="en" lang="en">EN</Link>}
              <i aria-hidden="true">/</i>
              {copy.locale === 'ru' ? <span aria-current="page">RU</span> : <Link href="/ru" hrefLang="ru" lang="ru">RU</Link>}
            </nav>
          </div>
          <TypingHeadline copy={copy.headline} />
          <p className="hero-text">{copy.introduction}</p>
        </div>

        <div
          className="education-card"
          aria-label={copy.education.ariaLabel}
          onPointerMove={tiltCard}
          onPointerLeave={resetTilt}
        >
          <div className="card-bar">
            <span>{copy.education.title}</span>
            <span>{copy.education.institution}</span>
          </div>
          <div className="education-body">
            <article className="degree">
              <span className="degree-index">01</span>
              <div>
                <small>{copy.education.bachelorLabel}</small>
                <h2>{copy.education.bachelorSubject}</h2>
              </div>
              <span className="degree-state complete">{copy.education.bachelorStatus}</span>
            </article>
            <article className="degree">
              <span className="degree-index">02</span>
              <div>
                <small>{copy.education.masterLabel}</small>
                <h2>{copy.education.masterSubject}</h2>
              </div>
              <span className="degree-state">{copy.education.masterStatus}</span>
            </article>
          </div>
        </div>

        <div className="social-section">
          <p className="social-title" id="social-title">{copy.socialTitle}</p>
